...
public class CourtPriceServiceImpl implements CourtPriceService {
        ...
        public CourtPriceResponse createPrice(CourtPriceRequest request) {
                ...
                List<CourtPrice> overlapping = courtPriceRepository.findOverlappingPrices(
                                request.getCourtId(), request.getDayType(), request.getStartTime(),
                                request.getEndTime());
                if (!overlapping.isEmpty()) {
                        throw new IllegalArgumentException(
                                        "Price already exists for this time range. Please update or delete the existing price first.");
                }
        ...
        public CourtPriceResponse updatePrice(Integer id, CourtPriceRequest request) {
                ...
                if (!courtPrice.getCourt().getId().equals(request.getCourtId())) {
                        BadmintonCourt court = badmintonCourtRepo.findById(request.getCourtId())
                                        .orElseThrow(
                                                        () -> new ResourceNotFoundException("Court not found with id: "
                                                                        + request.getCourtId()));
                        courtPrice.setCourt(court);
                }
        ...
        public List<CourtPriceResponse> createDefaultPricesForCourt(Integer courtId) {
                ...
                prices.add(createPriceEntity(court, DayType.WEEKDAY, LocalTime.of(6, 0), LocalTime.of(8, 0),
                                new BigDecimal("60000")));
                // Sáng: 08:00-11:00 - Giá bình thường
                prices.add(createPriceEntity(court, DayType.WEEKDAY, LocalTime.of(8, 0), LocalTime.of(11, 0),
                                new BigDecimal("80000")));
                // Trưa: 11:00-14:00 - Giá thấp
                prices.add(createPriceEntity(court, DayType.WEEKDAY, LocalTime.of(11, 0), LocalTime.of(14, 0),
                                new BigDecimal("60000")));
                // Chiều: 14:00-17:00 - Giá bình thường
                prices.add(createPriceEntity(court, DayType.WEEKDAY, LocalTime.of(14, 0), LocalTime.of(17, 0),
                                new BigDecimal("80000")));
                // Giờ vàng: 17:00-21:00 - Giá cao
                prices.add(createPriceEntity(court, DayType.WEEKDAY, LocalTime.of(17, 0), LocalTime.of(21, 0),
                                new BigDecimal("120000")));
                // Tối muộn: 21:00-22:00 - Giá bình thường
                prices.add(createPriceEntity(court, DayType.WEEKDAY, LocalTime.of(21, 0), LocalTime.of(22, 0),
                                new BigDecimal("80000")));

                // WEEKEND prices - Cao hơn ngày thường
                prices.add(createPriceEntity(court, DayType.WEEKEND, LocalTime.of(6, 0), LocalTime.of(8, 0),
                                new BigDecimal("80000")));
                prices.add(createPriceEntity(court, DayType.WEEKEND, LocalTime.of(8, 0), LocalTime.of(11, 0),
                                new BigDecimal("100000")));
                prices.add(createPriceEntity(court, DayType.WEEKEND, LocalTime.of(11, 0), LocalTime.of(14, 0),
                                new BigDecimal("80000")));
                prices.add(createPriceEntity(court, DayType.WEEKEND, LocalTime.of(14, 0), LocalTime.of(17, 0),
                                new BigDecimal("100000")));
                prices.add(createPriceEntity(court, DayType.WEEKEND, LocalTime.of(17, 0), LocalTime.of(21, 0),
                                new BigDecimal("150000")));
                prices.add(createPriceEntity(court, DayType.WEEKEND, LocalTime.of(21, 0), LocalTime.of(22, 0),
                                new BigDecimal("100000")));

                // HOLIDAY prices - Cao nhất, áp dụng cho ngày lễ
                prices.add(createPriceEntity(court, DayType.HOLIDAY, LocalTime.of(6, 0), LocalTime.of(8, 0),
                                new BigDecimal("100000")));
                prices.add(createPriceEntity(court, DayType.HOLIDAY, LocalTime.of(8, 0), LocalTime.of(11, 0),
                                new BigDecimal("120000")));
                prices.add(createPriceEntity(court, DayType.HOLIDAY, LocalTime.of(11, 0), LocalTime.of(14, 0),
                                new BigDecimal("100000")));
                prices.add(createPriceEntity(court, DayType.HOLIDAY, LocalTime.of(14, 0), LocalTime.of(17, 0),
                                new BigDecimal("120000")));
                prices.add(createPriceEntity(court, DayType.HOLIDAY, LocalTime.of(17, 0), LocalTime.of(21, 0),
                                new BigDecimal("180000")));
                prices.add(createPriceEntity(court, DayType.HOLIDAY, LocalTime.of(21, 0), LocalTime.of(22, 0),
                                new BigDecimal("120000")));
        ...
        @Override
        @Transactional
        public Map<String, Object> createPricesBatch(List<CourtPriceRequest> requests, boolean deleteExisting) {
                ...
                int skipCount = 0;
                int deleteCount = 0;
                List<CourtPriceResponse> createdPrices = new ArrayList<>();

                // Get unique court IDs from requests
                Set<Integer> courtIds = requests.stream()
                                .map(CourtPriceRequest::getCourtId)
                                .collect(java.util.stream.Collectors.toSet());

                // Delete existing prices if requested
                if (deleteExisting) {
                        for (Integer courtId : courtIds) {
                                List<CourtPrice> existing = courtPriceRepository.findByCourtId(courtId);
                                deleteCount += existing.size();
                                courtPriceRepository.deleteAll(existing);
                        }
                }

                // Create new prices
                for (CourtPriceRequest request : requests) {
                        try {
                                CourtPriceResponse response = createPrice(request);
                                createdPrices.add(response);
                                successCount++;
                        } catch (Exception e) {
                ...
                result.put("createdPrices", createdPrices);
                result.put("message", String.format("Đã tạo %d giá, bỏ qua %d trùng%s",
                                successCount, skipCount, deleteExisting ? ", xóa " + deleteCount + " giá cũ" : ""));
