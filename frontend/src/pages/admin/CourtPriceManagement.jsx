...
const CourtPriceManagement = () => {
  ...
  const [courtPrices, setCourtPricesLocal] = useState([]);
  ...
  useEffect(() => {
    filterCourtPrices();
  }, [selectedCourtId, selectedDayType, allCourtPrices]);
  ...
  const handleInitDefaultPrices = async () => {
    if (!selectedCourtId || selectedCourtId === 'all') {
      showToast('Vui lòng chọn sân cụ thể', 'error');
      return;
    }
    
    try {
      await courtPriceService.createDefaultPrices(parseInt(selectedCourtId));
      showToast('Tạo bảng giá mặc định thành công', 'success');
      await fetchCourtPrices();
    } catch (error) {
  ...
  const groupedPrices = courtPrices.reduce((acc, price) => {
      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
            ...
            <p className="text-3xl font-bold text-gray-900">{courtPrices.length}</p>
          </div>
          ...
          <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
            ...
            <p className="text-3xl font-bold text-gray-900">
              {courtPrices.filter(p => p.isActive).length}
            </p>
          </div>
          ...
          <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
            ...
            <p className="text-3xl font-bold text-gray-900">
              {courtPrices.filter(p => p.dayType === 'WEEKDAY').length}
            </p>
          </div>
          ...
          <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
            ...
            <p className="text-3xl font-bold text-gray-900">
              {courtPrices.filter(p => p.dayType === 'WEEKEND').length}
            </p>
          </div>
        </div>
